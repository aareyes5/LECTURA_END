...
def evaluar_texto(texto):
    ...
    niveles = {
        0: ["concentración normal", "me concentro bien", "sin problemas de concentración"],
        1: ["ligeros problemas de concentración", "ocasionalmente distraído"],
        2: ["dificultad ocasional para concentrarse", "de vez en cuando distraído"],
        3: ["problemas significativos de concentración", "frecuentemente distraído"],
        4: ["dificultad severa para concentrarse", "falta de concentración constante"],
        5: ["extremadamente difícil concentrarse", "concentración muy comprometida"],
        6: ["incapaz de concentrarse", "totalmente distraído", "problemas graves de concentración"]
    }
...
# Función para predecir el puntaje de la Pregunta 6
def predecir_puntaje(edad, sexo, puntaje6):
    # Crear un array con las entradas necesarias
    entrada = np.array([[edad, sexo, 0, 0, 0, 0, puntaje6, 0, 0, 0, 0]])  # Los valores de las preguntas 3-10 se inicializan en 0
    entrada_estandarizada = scaler.transform(entrada)
    predicciones = model.predict(entrada_estandarizada)
    puntaje_predicho = predicciones[0][0]  # La predicción de la Pregunta 6 es el primer valor
...
if __name__ == "__main__":
    ...
    audio_filename = os.path.join(audio_folder, f'Audio_6.wav')

    texto = audio_a_texto(audio_filename)
    puntaje_inicial = evaluar_texto(texto)
    print(f'Puntaje evaluado de la Pregunta 6: {puntaje_inicial}')
    ...
    print(f'Puntaje predicho para la Pregunta 6: {puntaje_predicho}')

    # Guardar el puntaje en el archivo de datos del usuario sin borrar los datos existentes
    guardar_puntaje_en_datos(datos_file_path, puntaje_predicho, pregunta_num=6)
