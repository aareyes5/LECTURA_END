...
def evaluar_texto(texto):
    ...
    niveles = {
        0: ["motivación normal", "interesado en actividades", "activo"],
        1: ["ligera disminución de la motivación", "ocasionalmente menos interesado"],
        2: ["motivación baja a veces", "poco interés en actividades"],
        3: ["falta de interés frecuente", "motivación notablemente baja"],
        4: ["gran falta de interés", "motivación muy baja", "sin ganas de hacer nada"],
        5: ["totalmente desmotivado", "falta de interés extrema", "sin energía"],
        6: ["completamente sin interés", "desesperanza total", "falta de motivación grave"]
    }
...
# Función para predecir el puntaje de la Pregunta 7
def predecir_puntaje(edad, sexo, puntaje7):
    # Crear un array con las entradas necesarias
    entrada = np.array([[edad, sexo, 0, 0, 0, 0, 0, puntaje7, 0, 0, 0]])  # Los valores de las preguntas 3-10 se inicializan en 0
...
if __name__ == "__main__":
    ...
    audio_filename = os.path.join(audio_folder, f'Audio_7.wav')

    texto = audio_a_texto(audio_filename)
    puntaje_inicial = evaluar_texto(texto)
    print(f'Puntaje evaluado de la Pregunta 7: {puntaje_inicial}')
    ...
    print(f'Puntaje predicho para la Pregunta 7: {puntaje_predicho}')

    # Guardar el puntaje en el archivo de datos del usuario sin borrar los datos existentes
    guardar_puntaje_en_datos(datos_file_path, puntaje_predicho, pregunta_num=7)
